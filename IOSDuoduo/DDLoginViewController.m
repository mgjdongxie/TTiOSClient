...
- (IBAction)login:(UIButton*)button
{
    ...
    userName = [userName length] != 0 ? userName : @"dongxie";
    NSString* password = _userPassTextField.text;
    password = [password length] != 0 ? password : @"dongxie";
    [[DDLoginModule instance] loginWithUsername:userName password:password success:^(DDUserEntity *user) {
        if (user) {
