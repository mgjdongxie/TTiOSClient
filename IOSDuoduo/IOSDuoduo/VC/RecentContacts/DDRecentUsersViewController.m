...
#import "DDRecentUsersViewController.h"
#import "DDRecentUserCell.h"
...
@interface DDRecentUsersViewController ()
...
@implementation DDRecentUsersViewController

+ (instancetype)shareInstance
{
    static DDRecentUsersViewController* g_recentUsersViewController;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        g_recentUsersViewController = [DDRecentUsersViewController new];
    });
...
- (DDRecentUserVCModule*)module
{
    if (!_module)
    {
        _module = [[DDRecentUserVCModule alloc] init];
    }
    return _module;
}
...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    DDRecentUserCell* cell = (DDRecentUserCell*)[tableView dequeueReusableCellWithIdentifier:cellIdentifier];
    if (!cell)
    {
        UINib* nib = [UINib nibWithNibName:@"DDRecentUserCell" bundle:nil];
        [tableView registerNib:nib forCellReuseIdentifier:cellIdentifier];
        cell = (DDRecentUserCell*)[tableView dequeueReusableCellWithIdentifier:cellIdentifier];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    DDRecentUserCell* cell = (DDRecentUserCell*)[tableView cellForRowAtIndexPath:indexPath];
    ...
    if ([self.items[row] isKindOfClass:[DDUserEntity class]]) {
        ...
        [[DDChattingMainViewController shareInstance] showChattingContentForSession:session];
        [self.navigationController pushViewController:[DDChattingMainViewController shareInstance] animated:YES];
    }else
    {
        DDGroupEntity *group = self.items[row];
    ...
    [[DDSundriesCenter instance] pushTaskToSerialQueue:^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            [self.items addObjectsFromArray:[[DDUserModule shareInstance] recentUsers]];
             [self.items addObjectsFromArray:[DDGroupModule instance].recentlyGroup];
            block(YES);
        });
    }];
}
...
-(void)loadRecentUserAndGroup
{
    ...
    dispatch_async(queue, ^{
        ...
        dispatch_group_enter(group);
        [[DDSundriesCenter instance] pushTaskToSerialQueue:^{
            DDRecentConactsAPI* recentContactsAPI = [[DDRecentConactsAPI alloc] init];
