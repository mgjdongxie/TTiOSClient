...
@implementation DDEmojiFaceView
...
{
	//row number
    NSArray* emotions = [DDEmotionsModule shareInstance].emotions;
	for (int i=0; i<DD_EMPTIONS_ROWS; i++) {
		...
		for (int y=0; y<DD_EMOTIONS_PERROW; y++) {
            ...
            if ((i * DD_EMOTIONS_PERROW + y + page * DD_MAC_EMOTIONS_COUNT_PERPAGE) > [emotions count]) {
                ...
                {
                    [button setImage:[UIImage imageWithName:@"dd_emoji_delete"] forState:UIControlStateNormal];
                    button.tag=10000;
