...
@interface DDEmotionsViewController ()
...
@implementation DDEmotionsViewController
...
- (void)viewDidLoad
{
    ...
    if (self.scrollView==nil) {
        ...
        for (int i=0; i<2; i++) {
            DDEmojiFaceView *fview=[[DDEmojiFaceView alloc] initWithFrame:CGRectMake(12+320*i, 15, facialViewWidth, facialViewHeight)];
    ...
    UIImage* backgroundImage = [UIImage imageWithName:@"dd_image_send"];
...
-(void)selectedFacialView:(NSString*)str
{
    if ([str isEqualToString:@"delete"]) {
        [[DDChattingMainViewController shareInstance] deleteEmojiFace];
        return;
    }
    [[DDChattingMainViewController shareInstance] insertEmojiFace:str];
}
