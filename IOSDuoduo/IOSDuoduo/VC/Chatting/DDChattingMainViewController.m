...
#import "DDRecordingView.h"
#import "DDAnalysisImage.h"
...
@interface DDChattingMainViewController ()<UIGestureRecognizerDelegate>
...
- (void)p_setContentEdgeInset:(UIEdgeInsets)edgeInset;
@end

@implementation DDChattingMainViewController
{
    DDTouchDownGestureRecognizer* _touchDownGestureRecognizer;
    ...
    BOOL _scrollloginEnable;
}
+(instancetype )shareInstance
{
    static dispatch_once_t onceToken;
    static DDChattingMainViewController *_sharedManager = nil;
    dispatch_once(&onceToken, ^{
        _sharedManager = [DDChattingMainViewController new];
    });
...
- (void)viewDidLoad
{
    ...
    _originalTableViewContentInset = self.tableView.contentInset;
     ...
     self.activity.frame=CGRectMake(20, 50, 20, 20);
...
- (DDChattingModule*)module
{
    if (!_module)
    {
        _module = [[DDChattingModule alloc] init];
    }
    return _module;
}
...
{
    DDEmotionsModule* emotionModule = [DDEmotionsModule shareInstance];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    static BOOL loadingHistory = NO;
     if (scrollView.contentOffset.y < -10 && [self.activity isAnimating] && [self.module.showingMessages count] > 0 && !loadingHistory && _scrollloginEnable)
     {
         loadingHistory = YES;
             ...
             [self.module loadMoreHistoryCompletion:^(NSUInteger addCount,NSError *error) {
                 ...
                 if ([sessionID isEqualToString:self.module.sessionEntity.sessionID])
                 {
                     ...
                     if (addCount < DD_PAGE_ITEM_COUNT)
                     {
                         ...
                         [self p_setContentEdgeInset:UIEdgeInsetsMake(10, 0, 0, 0)];
                         self.tableView.scrollIndicatorInsets = UIEdgeInsetsMake(0, 0, 0, 0);
...
- (void)showChattingContentForSession:(DDSessionEntity*)session
{
    ...
    [[DDDatabaseUtil instance] insertMessages:unreadMessages success:^{
        ...
        [sendMessageReadACKAPI requestWithObject:session.sessionID Completion:nil];
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    [_tableView reloadData];
                    //[self p_setContentEdgeInset:UIEdgeInsetsMake(40, 0, 0, 0)];
                    [self scrollToBottomAnimated:NO];
                });
            }
            else
...
- (void)n_receiveMessage:(NSNotification*)notification
{
    ...
    [DDAnalysisImage analysisImage:message Block:^(NSMutableArray *array) {
        for (DDMessageEntity *msg in array) {
            ...
            {
//                UIImage* image = [UIImage imageWithName:@"dd_has_unread_message"];
...
- (void)p_clickThRecordButton:(UIButton*)button
{
    switch (button.tag) {
        case DDVoiceInput:
            //开始录音
            [self p_hideBottomComponent];
            [button setImage:[UIImage imageWithName:@"dd_input_normal"] forState:UIControlStateNormal];
        ...
        case DDTextInput:
            //开始输入文字
            [button setImage:[UIImage imageWithName:@"dd_record_normal"] forState:UIControlStateNormal];
...
- (void)p_setContentEdgeInset:(UIEdgeInsets)edgeInset
{
    _scrollloginEnable = NO;
    [self.tableView setContentInset:edgeInset];
    _scrollloginEnable = YES;
}
...
@implementation DDChattingMainViewController(ChattingInput)
...
- (void)initialInput
{
    ...
    _touchDownGestureRecognizer = [[DDTouchDownGestureRecognizer alloc] initWithTarget:self action:nil];
    __weak DDChattingMainViewController* weakSelf = self;
    ...
    _recordingView = [[DDRecordingView alloc] initWithState:DDShowVolumnState];
    [_recordingView setHidden:YES];
...
{
    [_recordButton setImage:[UIImage imageWithName:@"dd_record_normal"] forState:UIControlStateNormal];
    ...
    {
        self.ddUtility = [DDChatUtilityViewController new];
...
-(IBAction)showEmotions:(id)sender
{
    ...
    if (self.emotions == nil) {
        self.emotions = [DDEmotionsViewController new];
