...
@property(nonatomic,strong)DDChattingEditModule *model;
...
- (void)viewDidLoad
{
    ...
    UIView *lineView = [[UIView alloc] initWithFrame:CGRectMake(0, self.gridView.frame.size.height, 320, 1)];
    ...
    UIView *view = [[UIView alloc] initWithFrame:self.gridView.frame];
...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([self.plusImage isEqual:user]) {
        cell.tag=100;
        cell.personIcon.image = [UIImage imageWithName:user.avatar];
        
    }else if([self.minus_sign isEqual:user])
    {
        cell.tag=101;
         cell.personIcon.image= [UIImage imageWithName:user.avatar];
    }else
    {
        [cell.personIcon sd_setImageWithURL:[NSURL URLWithString:user.avatar] placeholderImage:[UIImage imageWithName:@"user_placeholder"]];
...
-(void)createGroup
{
    ...
    [creatGroup requestWithObject:array Completion:^(id response, NSError *error) {
        ...
        [DDChattingMainViewController shareInstance].module.sessionEntity =session;
        [self loadGroupUsers];
    }];
}
