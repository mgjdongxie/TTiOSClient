...
- (instancetype)init
{
    ...
    if (self) {
        ...
        self.recentlyGroup = [NSMutableArray new];
        self.recentGroupCount=0;
        [self registerAPI];
    }
    return self;
}
...
{
    [self.recentlyGroup removeObjectsInArray:array];
    [self.recentlyGroup insertObjects:array atIndexes:[NSIndexSet indexSetWithIndexesInRange:NSMakeRange(0, [array count])]];
    [array enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        [self addGroup:obj];
    }];
    self.recentGroupCount = self.recentlyGroup.count;
}
@end
