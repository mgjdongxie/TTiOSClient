...
- (void)addUnreadMessage:(DDMessageEntity*)message
{
    ...
    {
        if ([[_unreadMessages allKeys] containsObject:message.toUserID])
        {
            NSMutableArray* unreadMessage = _unreadMessages[message.toUserID];
            [unreadMessage addObject:message];
        }
        else
        {
            NSMutableArray* unreadMessages = [[NSMutableArray alloc] init];
            [unreadMessages addObject:message];
            [_unreadMessages setObject:unreadMessages forKey:message.toUserID];
        }
    }
